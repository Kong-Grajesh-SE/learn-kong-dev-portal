...
hero:
  name: "Specialist Bootcamps"
  text: "Go Deeper with Kong"
  tagline: "Focused, hands-on bootcamps covering AI Gateway, Agentic AI, APIOps, API testing, and more."
  image:
    src: /kong-logomark-lime.svg
    alt: Kong Specialist Bootcamps

features:
  - icon: 🏠
    title: "API Gateway Bootcamp ↗"
    details: "Core → Auth → Traffic → Transform → Observe → Enterprise. Seven hands-on modules on Konnect covering every essential Kong Gateway concept."
    link: https://kong-grajesh-se.github.io/learn-kong-gateway/
    linkText: Go to API Gateway →
